...
namespace Loadlibrayy.Injection
{
    public unsafe class LoadLibraryInjection : IInjectionMethod
    {
        ...
        public bool InjectImage(string imagePath)
        {
            ...
            UInt32 loadLibraryPointer = (UInt32)NTM.GetModuleFunction("kernel32.dll", "LoadLibraryA");

            // ALLOCATE IMAGE PATH IN TARGET PROCESS
            var rawImagePath = Encoding.Default.GetBytes(imagePath);
            UInt32 allocatedImagePath = (UInt32)TargetProcess.AllocateAndWrite(rawImagePath, NT.AllocationType.Commit | NT.AllocationType.Reserve, NT.MemoryProtection.ReadWrite);
