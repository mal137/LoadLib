...
namespace Loadlibrayy.Helpers
{
    ...
    {
        public static byte[] CallLoadLibrary(UInt32 allocatedImagePath, UInt32 loadLibraryPointer)
        {
            ...
            {
                *(UInt32*)(shellcodePointer + 19) = allocatedImagePath;
                *(UInt32*)(shellcodePointer + 29) = loadLibraryPointer;
            }

            return shellcode;
        }

        public static byte[] CallDllMain(UInt32 remoteImage, UInt32 entrypoint, bool hijackSafe)
        {
            byte[] shellcode;
            ...
            if (hijackSafe)
            {
                ...
                fixed (byte* shellcodePointer = shellcode)
                {
                    *(UInt32*)(shellcodePointer + 19) = remoteImage;
                    *(UInt32*)(shellcodePointer + 39) = entrypoint;
                }
            }
            else
            {
                ...
                fixed (byte* shellcodePointer = shellcode)
                {
                    *(UInt32*)(shellcodePointer + 6) = remoteImage;
                    *(UInt32*)(shellcodePointer + 26) = entrypoint;
                }
            }
